# ArisChat Extension System 設計書

## 概要

外部GitHubリポジトリをインストールし、ArisChatに**新しい画面・機能そのもの**を追加できるプラグインアーキテクチャ。
...
│   ├── arischat-ext-claude-code/  # git clone されたリポジトリ
...
```json
{
  "extensions": [
    {
      "id": "arischat-ext-claude-code",
      "source": "https://github.com/user/arischat-ext-claude-code",
      "version": "1.0.0",
...
拡張は npm パッケージとして構成する。`arischat` フィールドに拡張固有の設定を記述。

```json
{
  "name": "arischat-ext-claude-code",
  "version": "1.0.0",
  ...
  "devDependencies": {
    "@anthropic-ai/arischat-extension-api": "^1.0.0",
    "esbuild": "^0.20.0"
  }
}
```
...
| フィールド | 必須 | 説明 |
|-----------|------|------|
| `arischat.displayName` | Yes | UI表示名 |
| `arischat.icon` | Yes | サイドバーアイコン（emoji or Lucide icon名） |
| `arischat.permissions` | Yes | 要求する権限 |
| `arischat.main` | No | Main Process エントリ（Node.js） |
| `arischat.renderer` | Yes | Renderer エントリ（React コンポーネント export） |
| `arischat.pages` | No | 登録するページ |
| `arischat.settings` | No | 設定画面に追加するパネル |
| `arischat.minAppVersion` | No | 必要な最低ArisChatバージョン |
...
│                               ▼                      │
│  ┌──────────────┐  ┌──────────────────────────────┐ │
│  │  ArisChat    │  │  Extension Renderer Entry     │ │
│  │  App.tsx     │──│  (renderer.js)                │ │
│  │  Router      │  │                               │ │
...
interface ExtensionContext {
  ...
  settings: {
    get(): Promise<Partial<ArisChatSettings>>;  // APIキーはマスク
    set(patch: Partial<ArisChatSettings>): Promise<void>;
  };
...
import type { ExtensionContext } from '@anthropic-ai/arischat-extension-api';
...
import type { ExtensionRendererAPI } from '@anthropic-ai/arischat-extension-api';
...
2. package.json を読み取り、arischat フィールドをバリデーション
...
│ 拡張機能をインストール                    │
│                                          │
│ Claude Code (arischat-ext-claude-code)   │
│ by: user                                 │
│                                          │
│ この拡張機能は以下の権限を要求します:     │
│                                          │
...
class ExtensionManager {
  ...
  async load(entry: RegistryEntry) {
    ...
    const arischat = manifest.arischat;

    // Main Entry のロード
    if (arischat.main) {
      const mainPath = path.join(this.extensionsDir, entry.id, arischat.main);
      ...
      {extensions.length > 0 && (
        <div className="sidebar-section">
          ...
          {extensions.flatMap(ext =>
            ext.manifest.arischat.pages?.map(page => (
              <button
                key={`${ext.id}:${page.id}`}
...
async function loadRendererModule(ext: ExtensionInfo) {
  ...
  const code = await window.arisChatAPI.extensions.readRendererEntry(ext.id);
...
build({
  ...
  platform: 'browser',
  external: ['react', 'react-dom'],  // ArisChat が提供するので除外
  jsx: 'automatic',
});
...
拡張専用の IPC チャンネルは、既存の ArisChat IPC と混在しないようにプレフィックスで隔離する。

```typescript
// 拡張 IPC のチャンネル名規則
// ext:{extensionId}:{channel}

// 例: arischat-ext-claude-code の "run-command" チャンネル
// → "ext:arischat-ext-claude-code:run-command"
...
function sanitizeSettings(settings: ArisChatSettings): Partial<ArisChatSettings> {
...
- `window.arisChatAPI` に直接アクセスできない（Renderer Entry 側にはノーアクセス設計）
...
### `@anthropic-ai/arischat-extension-api`
...
| # | 内容 |
|---|------|
| 17 | `@anthropic-ai/arischat-extension-api` 型定義パッケージ作成 |
...
`npm create arischat-extension` で生成されるテンプレート:

```
arischat-ext-my-extension/
├── package.json
├── tsconfig.json
├── src/
...
- ArisChat が React で構築されているため、拡張も React で書くのが最も自然
